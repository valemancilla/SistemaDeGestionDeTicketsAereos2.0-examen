namespace SistemaDeGestionDeTicketsAereos.src.modules.CheckIn.Domain.valueObject;

// Value Object para el ID del check-in
public sealed record CheckInId
{
    // El valor entero del ID
    public int Value { get; }

    // Constructor privado: solo se crea a través del método Create
    private CheckInId(int value) => Value = value;

    // Valida que el ID no sea negativo
    public static CheckInId Create(int value)
    {
        if (value < 0)
            throw new ArgumentException("CheckInId must be greater than 0.", nameof(value));

        return new CheckInId(value);
    }

    public override string ToString() => Value.ToString();
}
