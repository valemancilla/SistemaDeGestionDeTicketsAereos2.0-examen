...
public sealed class CreateFlightUseCase
{
    ...
    public async Task<Flight> ExecuteAsync(string number, DateOnly date, TimeOnly departureTime, TimeOnly arrivalTime, int totalCapacity, int availableSeats, int idRoute, int idAircraft, int idStatus, int idCrew, int? idFare, string? boardingGate = null, CancellationToken ct = default)
    {
