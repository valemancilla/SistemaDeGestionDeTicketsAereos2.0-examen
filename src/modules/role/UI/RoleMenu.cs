...
namespace SistemaDeGestionDeTicketsAereos.src.modules.role.UI;
...
public sealed class RoleMenu
{
    public async Task RunAsync(CancellationToken ct = default)
    {
        ...
        {
            Console.Clear();
            AnsiConsole.Write(new Rule("[green]GESTIÓN DE ROLES DE USUARIO[/]").Centered());
    ...
    private static async Task ListAsync(CancellationToken ct)
    {
        ...
        var items = await new GetAllRolesUseCase(new RoleRepository(context)).ExecuteAsync(ct);
        if (!items.Any()) { AnsiConsole.MarkupLine("[yellow]No hay roles registrados.[/]"); }
        else
        {
    ...
    {
        Console.Clear();
        AnsiConsole.Write(new Rule("[yellow]CREAR ROL DE USUARIO[/]").Centered());
        if (!AnsiConsole.Confirm("¿Deseas crear un rol?", true))
            return;
        var name = AnsiConsole.Ask<string>("Nombre (ej: Administrador, Cliente, Agente):");
        try
        {
            using var context = DbContextFactory.Create();
            var result = await new CreateRoleUseCase(new RoleRepository(context)).ExecuteAsync(name, ct);
            await context.SaveChangesAsync(ct);

            var createdId = (await new GetAllRolesUseCase(new RoleRepository(context)).ExecuteAsync(ct))
    ...
    {
        Console.Clear();
        AnsiConsole.Write(new Rule("[yellow]ACTUALIZAR ROL[/]").Centered());
        ...
        {
            using var context = DbContextFactory.Create();
            await new UpdateRoleUseCase(new RoleRepository(context)).ExecuteAsync(id, name, ct);
            await context.SaveChangesAsync(ct);
    ...
    {
        Console.Clear();
        AnsiConsole.Write(new Rule("[yellow]ELIMINAR ROL[/]").Centered());
        var id = AnsiConsole.Prompt(
        ...
        {
            using var context = DbContextFactory.Create();
            var deleted = await new DeleteRoleUseCase(new RoleRepository(context)).ExecuteAsync(id, ct);
            await context.SaveChangesAsync(ct);
